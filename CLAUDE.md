...
Routes require **permissions, not roles** (`authorize(req, "license:revoke")`); the role→permission matrix lives only in `domain/rbac.ts`. The SPAs mirror it via `/admin/me` purely to hide controls — the server is the sole enforcer. Customer endpoints (`/api/v1/customer/*`) are scoped by `requireOwnedLicense`, which returns NOT_FOUND (never 403) on cross-customer access to avoid existence leaks. Public client endpoints (activate/validate/trial/floating/offline/deactivate) are unauthenticated but rate-limited (`enforceRateLimit`); `/deactivate` authenticates by proof-of-possession of a validly signed token.
