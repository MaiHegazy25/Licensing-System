...
| T4 | **Leaked activation code** | High-entropy codes; only HMAC(pepper) hash stored; single/max-use; rate limiting _(planned)_; revocable | ✅ hashing/use-limit; ⏳ rate limit |
| T5 | **Stolen token** | Short TTL forces re-validation; revocation kills it server-side; audience/issuer bound; tokenId for tracing | ✅ implemented |
...
| T8 | **API abuse / enumeration / brute force** | Opaque UUID ids; uniform error shapes; rate limiting + security events _(planned)_; constant-time secret compares | ✅ ids/compares; ⏳ rate limit |
